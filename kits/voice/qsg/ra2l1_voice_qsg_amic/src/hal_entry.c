...
void hal_entry(void)
{
    ...
    /* Set the DCT to capture from ADC registers into amic_buf */
    err = R_DTC_Reset(&g_transfer0_ctrl, (void *) R_ADC0->ADDR, amic_buf[amic_idx], AMIC_BUF_SIZE);
    if (FSP_SUCCESS != err)
    {
        __BKPT(0);
    }
...
void g_transfer0_cb(adc_callback_args_t * p_args)
{
    /* Change index of the active write buffer */
    amic_idx ^= 1;

    /* Start subsequent ADC capture */
    R_DTC_Reset(&g_transfer0_ctrl, (void *) R_ADC0->ADDR, amic_buf[amic_idx], AMIC_BUF_SIZE);
